...
public class Rol {
    ...
    private List<Permiso> permissions; //Lazy
    ...
    public void setAdmin(boolean value) {
        throw new Error("Un Rol no puede concederse permissions de administraci�n a s� mismo.");
    }
    ...
    private List<Permiso> getPermissions() {
        if (permissions == null) permissions = Permiso.ListaPermisosRol(this.rolName);
        return permissions;
    }

    public boolean Acceso(String pantalla) {
        for (Permiso p : this.getPermissions()) {
    ...
    public void AddPermiso(Permiso p) {
        if (!this.getPermissions().contains(p)) {
            permissions.add(p);
        }
    }
